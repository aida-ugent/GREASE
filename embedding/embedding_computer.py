...
from embedding.cne_method.cne_debugged import ConditionalNetworkEmbedding
from embedding.cne_method.bg_dist_partite import BgDistBuilder
...
class NetworkEmbeddingMethod:
    ...
    def set(self, graph=None, embedding=None, block_mask=None, node_types_dict=None, ne_method=None):
        ...
        if ne_method is not None:
            self.ne_method = ne_method

    def apply_reembedding(self, graph, selected_links, max_iter=250, lr=0.01):
        g = graph.copy()
        for l in selected_links:
            g.add_edge(l[0], l[1])
            g.add_edge(l[1], l[0])
        new_cne, embeddings_new = self.compute_embeddings_with_init_embeddings(
            g, recompute_prior=True, links_to_small_add=None, epsilon=0, max_iter=max_iter, ftol=1e-8, lr=lr)
